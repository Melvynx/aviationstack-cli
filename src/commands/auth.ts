...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  aviationstack-cli auth set sk-abc123xyz")
  .action((token: string) => {
    setToken(token);
...
authCommand
  ...
  .addHelpText("after", "\nExample:\n  aviationstack-cli auth show\n  aviationstack-cli auth show --raw")
  .action((opts: { raw?: boolean }) => {
    if (!hasToken()) {
      log.warn("No token configured. Run: aviationstack-cli auth set <token>");
      return;
    }
